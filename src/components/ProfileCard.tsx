...
export default function ProfileCard({ profile, index }: { profile: LeaderProfile; index: number }) {
  return (
    <motion.article
      ...
      className="group"
    >
      <div className="relative overflow-hidden rounded-2xl bg-white/10 shadow-[0_1px_2px_rgba(0,0,0,0.08)] ring-1 ring-white/15 transition-shadow duration-500 group-hover:shadow-[0_24px_48px_-20px_rgba(0,0,0,0.45)]">
        <img
      ...
      <div className="mt-5">
        ...
        <p className="mt-2.5 flex items-center gap-1.5 text-sm text-white/60">
          <MapPin size={14} strokeWidth={2} />
          {profile.region}
        </p>
        <p className="mt-3 text-sm leading-relaxed text-white/75">{profile.description}</p>
      </div>
    </motion.article>
  );
}
